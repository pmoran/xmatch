  ...
  module XML
    ...
    class Element
      ...
      private
        ...
        def children_match?(other)
          ...
          @matcher.record(self, other, match, "expected #{children.size} children, got #{other.children.size}")
          match
        end

        def name_matches?(other)
          match = name == other.name
          @matcher.record(self, other, match, "expected element '#{name}', got '#{other.name}'")
          match
        end
        ...
        def attributes_match?(other)
          ...
          if match
            ...
            @matcher.record(self, other, match, "expected #{attributes.size} attributes, got #{other.attributes.size}")
          end
          match
        end
    ...
    class Text
      def match?(other, matcher)
        ...
        matcher.record(self, other, match, "expected '#{content}', got '#{other.content}'")
        match
      end
    end
    ...
    class Attr
      def match?(other, matcher)
        if other.nil?
          ...
          matcher.record(parent, nil, false, "expected attribute missing")
          return false
        end

        custom_matcher = matcher.custom_matchers[path]
        match = custom_matcher ? custom_matcher.call(other) : (value == other.value)
        matcher.record(parent, other.parent, match, "attribute '#{name}' expected '#{value}', got '#{other.value}'")
        match
      end
    end
